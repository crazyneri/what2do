export default function EmptyRefinements() {
    return (
        <div className="drag-space">
            <p className="instructions-bold">
                Decide what you want to do - the closer to the top, the more you
                want to do it!
            </p>
            <br />

            <p>
                1. <span className="instructions-bold">Choose</span> the main
                category by dragging it into the centre.
            </p>
            <p>
                2. <span className="instructions-bold">Click</span> on refine to
                add the sub-categories you want.
            </p>
            <p>
                3. Click the
                <span className="instructions-bold"> close button</span> to
                close the refinements.
            </p>
            <p>
                4. When you've chosen, press
                <span className="instructions-bold"> search</span>!
            </p>
        </div>
    );
}
